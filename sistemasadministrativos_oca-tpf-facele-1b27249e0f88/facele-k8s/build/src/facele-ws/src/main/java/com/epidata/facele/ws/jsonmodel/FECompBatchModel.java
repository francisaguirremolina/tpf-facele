...
public class FECompBatchModel {
    ...
    public TR_FEComprobantes asTR_FEComprobantes() throws Exception {
        ...
        completeAndValidateCab(l_toReturn);

        return l_toReturn;
    }

    private void completeAndValidateCab(TR_FEComprobantes l_toReturn) throws Exception {
        if (l_toReturn.getDocNro() == null) {
            l_toReturn.setDocNro(0l);
        } // end if
        l_toReturn.setImpTotal(0.0); // debe recalcularse
        if (l_toReturn.getImpTotConc() == null) {
            l_toReturn.setImpTotConc(0.0);
        }// end if
        l_toReturn.setImpNeto(0.0); // debe recalcularse
        if (l_toReturn.getImpOpEx() == null) {
            l_toReturn.setImpOpEx(0.0);
        }// end if
        l_toReturn.setImpTrib(0.0); // debe recalcularse
        l_toReturn.setImpIVA(0.0); // debe recalcularse
        if (l_toReturn.getMonId() == null) {
            l_toReturn.setMonId("PES");
            l_toReturn.setMonCotiz(1.0);
        } // end if

        if (l_toReturn.getResultado() == null) {
            l_toReturn.setResultado("P");
        } // end if
        if (l_toReturn.getEstadoPDF()== null) {
            l_toReturn.setEstadoPDF("P");
        } // end if
    ...
    public List<TR_FEComprobantesAsoc> asTR_FEComprobantesAsoc() {
        ...
        for (FEComprobantesAsocModel l_srcModel : m_comprobantesAsoc) {
            ...
            l_toAdd.setCbteCuit(l_srcModel.getCbteCuit());
    ...
    public List<TR_FEComprobantesDet> asTR_FEComprobantesDet() {
        ...
        for (FEComprobantesDetModel l_srcModel : m_comprobantesDet) {
            ...
            l_toAdd.setSubtotal(0.00);  // Debe ser recalculado
    ...
    public List<TR_FEComprobantesIva> asTR_FEComprobantesIva() {
        ...
        for (FEComprobantesIvaModel l_srcModel : m_comprobantesIva) {
            ...
            l_toAdd.setImporte(0.00); // debe ser recalculado
    ...
    public List<TR_FEXPermisos> asTR_FEXPermisos() {
        ...
        for (FEComprobantesPermisosModel l_srcModel : m_comprobantesPermisos) {
            ...
            l_toAdd.setIdFEXDstPais(l_srcModel.getDestMercaderia());
